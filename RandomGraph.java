...
public class RandomGraph {
	 ...
	 * @param zeta   - random decimal between 0 and 1 that we use to pick a
	 ...
	 */
	public ArrayList<Integer> random(int n, ArrayList<BigDecimal> graphs, BigDecimal zeta,
			HashMap<ArrayList<Integer>, BigDecimal> map) {
		BigDecimal probabilitySum = new BigDecimal("0");
		int k = 0;
		ArrayList<Integer> pi = new ArrayList<>();
		while ((probabilitySum.max(zeta) == zeta) && k <= n) {
			if (k >= 2 || k == 0) {
				ArrayList<ArrayList<Integer>> remainingPartition = part.partitionWithNoOnes(k);
				for (ArrayList<Integer> part : remainingPartition) {
					ArrayList<Integer> tempPart = new ArrayList<>();
					for (int i = 0; i < (n - k); i++) {
						part.add(1);
					}
					tempPart.addAll(part);
					BigDecimal probOfPartition = computeProbability(tempPart, n, graphs);
					probabilitySum = probabilitySum.add(probOfPartition);
					map.put(tempPart, probabilitySum);
	 ...
	 * @param n         - number of vertices
	 * @return returns the product function in denominator of the weight of the
	 *         conjugacy class
	 */
